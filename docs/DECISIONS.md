...
The brief allows VitePress or Astro Starlight. Both pull a large node_modules tree and a framework runtime into a project whose stated principle is "plain web stack, no framework". We generate the docs as static HTML with `tools/build-docs.js`, which reads the JSDoc comments, the cookbook Markdown, and the Qubi reference. The docs share `styles/tokens.css` with the playground. This is the same outcome (API reference from JSDoc, cookbook, reference pages) with no framework.
...
## Web: browser video is WebCodecs only, muxed by mp4-muxer and webm-muxer

ffmpeg.wasm (31 MB, GPL core) was removed. Every browser video export encodes with WebCodecs. MP4 goes through mp4-muxer with the first video codec the browser offers from H.264, AV1, and VP9, and AAC or else Opus audio; the export panel notes when the MP4 is not H.264 (the Playwright Chromium has no H.264 or AAC encoder, so it writes AV1 with Opus). Opaque WebM goes through webm-muxer with VP9, VP8, or AV1. webm-muxer cannot attach alpha to video blocks, so a transparent WebM uses our `src/playground/export/webm.js`: when the encoder keeps alpha itself its side data is written as BlockAdditions, and otherwise (the case in current Chromium) a second encoder encodes an I420 frame whose luma plane is the alpha channel, which is the layout libvpx and browsers decode. `tests/unit/web-muxers.test.js` checks that layout against libvpx. A browser with no WebCodecs encoder gets a clear message pointing to PNG, GIF, APNG, or the CLI.
...
## Editor and quantum first; no gallery

The product centers on the visual editor and the quantum views. The example gallery (42 graded scenes with poster renders), the theme contact sheets, and the playground's gallery panel took about 68 MB of rendered images and pulled attention toward general math animation. They are removed. Code mode stays as the secondary path, opening on a starter scene with the inspector beside the preview. The remaining examples are the quantum scenes plus the few that the golden tests, the benchmark, and the embed demo use.

## The app is the editor; code scenes belong to the CLI and the embed element

The Code tab (JavaScript and Qubi source panes, Run, Problems, the preview inspector, and the Editor/Code switch) is removed, with the runtime's Qubi source kind and its pick, inspect, still, and options requests. Scenes written in code still render with `qgfx` and play in `<qubibyte-scene>`, which is why the runtime keeps the module source kind and import rewriting. An old code or Qubi link opens the editor with a note. The remaining examples are the ones the golden tests, the benchmark, and the embed demo load.
